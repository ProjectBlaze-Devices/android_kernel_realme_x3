...
static int msm_drm_init(struct device *dev, struct drm_driver *drv)
{
	...
	for (i = 0; i < priv->num_crtcs; i++) {
		...
		if (i == 0) {
			...
			pr_info("crtc_commit:%d to big cluster", priv->disp_thread[i].crtc_id);
		} else {
			priv->disp_thread[i].thread =
				kthread_run(kthread_worker_fn,
					&priv->disp_thread[i].worker,
					"crtc_commit:%d", priv->disp_thread[i].crtc_id);
			pr_info("crtc_commit:%d to little cluster", priv->disp_thread[i].crtc_id);
		}
		...
		if (i == 0) {
			...
			pr_info("crtc_event:%d to big cluster", priv->event_thread[i].crtc_id);
		} else {
			priv->event_thread[i].thread =
				kthread_run(kthread_worker_fn,
					&priv->event_thread[i].worker,
					"crtc_event:%d", priv->event_thread[i].crtc_id);
			pr_info("crtc_event:%d to little cluster", priv->event_thread[i].crtc_id);
		}
